...
class Meta(BaseModel):
    ...
    order: int = 1000
    # Short sidebar label. Default = name — set this instead of shortening
    # `name:`, which would also churn <title>, og:title and the llms.txt
    # heading.
...
def _expand_source_directives(markdown_content: str) -> str:
    ...
    is documentation showing the syntax, not a directive (the template's
    docs/example and docs/directives both teach `.. source::` inside
    ```markdown fences). Expanding it injects a ```python fence inside the
    already-open fence, which CLOSES it early — from there the inlined
    file renders as markdown, every `# comment` line becomes an <h1>, and
    the machine lane of the page serves broken structure (found 2026-08-23
    by the single-h1 pin in tests/test_pages.py; the browser lane was
    never affected because markdown2dash parses fences properly).
    """
