#!/usr/bin/env python
"""Dash compatibility matrix for dash-email.

`setup.py` claims `dash>=4.1`. This script is what turns that claim into
evidence: it builds one throwaway virtualenv per Dash version, installs the
docs site into each, and runs `scripts/smoke_test.py` there — the component
surface, page registration, layout render + JSON serialisation of every
example, and an HTTP sweep of every route through Flask's test client.
    ...
    python scripts/compat_matrix.py --local               # no network (see below)
...
-----------------------
The smoke test drives the app through Flask's in-process test client, which
needs no socket and no browser and still exercises routing, layout
construction and serialisation — the layer where a Dash version bump actually
breaks things. Browser-side checks (components actually painting, console
...
# Where a virtualenv actually lives, relative to a sibling project directory.
# These are depth-bounded on purpose. The obvious `*/**/site-packages/...`
# recursive glob descends into every node_modules tree under the search root,
# which on a machine with a hundred JS projects takes minutes and looks exactly
# like a hang — no output, because the first log line comes after discovery.
VENV_GLOBS = (
...
def discover_interpreters(search_root: Path) -> dict[str, Path]:
    ...
    Interpreters whose Python minor version matches THIS one are preferred.
    `_compat_runner.py` lends them this project's site-packages, and a
    compiled extension built for cpython-312 will not import under 3.13 —
    `pydantic_core` is the one that bites here. Picking a same-version venv
    keeps the donor usable; the runner degrades safely when none exists.
    """
    own = f"python{sys.version_info.major}.{sys.version_info.minor}"
    found: dict[str, Path] = {}
    matched: dict[str, bool] = {}
    for pattern in VENV_GLOBS:
        for site in search_root.glob(pattern):
            if not site.is_dir():
                continue
            # .../<venv>/lib/python3.x/site-packages
            python = site.parent.parent.parent / "bin" / "python"
            if not python.exists():
                continue
            same = site.parent.name == own
            for info in site.glob("dash-*.dist-info"):
                version = info.name[len("dash-"):-len(".dist-info")]
                # `dash_ag_grid-*.dist-info` and friends must not match.
                if not (info.is_dir() and version and version[0].isdigit()):
                    continue
                # First match wins, but a same-Python one always upgrades it.
                if version not in found or (same and not matched.get(version)):
                    found[version] = python
                    matched[version] = same
    mismatched = [v for v, ok in matched.items() if not ok]
    if mismatched:
        log(f"NOTE: no {own} venv for dash {', '.join(sorted(mismatched))} — "
            "those rows run with the target's own libraries, not this project's")
    return found
...
def make_venv(version: str) -> tuple[Path, Path] | None:
    """Create a venv with `dash==<version>` + the docs requirements."""
    ...
    log(f"{version}: installing dash=={version}")
    r = run([str(py), "-m", "pip", "install", "-q", f"dash=={version}"])
    if r.returncode:
        ...
        with sync_playwright() as pw:
            ...
            for path in pages_json:
                ...
                try:
                    ...
                    page.wait_for_timeout(1500)  # let the email preview paint
