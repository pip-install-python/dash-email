...
"""Post-deploy checks against a *live* satellite.
    ...
    python scripts/smoke_live.py https://email.2plot.dev
...
def main(base: str) -> int:
    ...
    # stanza at all — absent or Allow are both the allow shape.
    for agent in ("ClaudeBot", "GPTBot"):
        got = robots_rule(agent)
        check(
            f"/robots.txt {agent} not Disallowed (posture)",
            got != "Disallow: /",
            f"got {got}: this host still walls training crawlers (sync item 15)",
        )

    # The repository link must RESOLVE (item 16, ported from the template's
    # 1.6.41; pannellum's finding: icon, JSON-LD codeRepository and
    # llms-github-repo meta all spelled a repo wrong — a live 404 that
    # "profile vs repo" framing never caught). GitHub answers GET on a repo
    # page with 200; a wrong slug is 404. Its OWN request, not `fetch`:
    # every fork's test suite stubs `fetch` with a fixed signature and
    # routes it to the in-process app, and GitHub is not this app.
    # Unreachable (a sandbox with no egress) is a notice, not a red; a
    # reachable 404 is red.
    try:
