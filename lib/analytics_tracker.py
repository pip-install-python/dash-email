...
additionally carry ``{vendor_key, vendor_class, verified, lane}`` since sync
item 12; human rows are unchanged.

Since item 12 the same file holds a SECOND table, ``reads``: one row per
...
class AnalyticsTracker:
    ...
    def _write(self, pending, reads=()):
        ...
        try:
            ...
            # A ledger written before item 12 has no `reads`; absence is empty.
