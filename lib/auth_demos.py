...
DEMOS: dict[str, dict] = {
    # This site's hero: the full newsletter template from the Full Templates
    # page, rendered live inside the sign-in card — the strongest "this is
    # what you get" teaser a component library's gate can make. The module is
    # the same `.. exec::` example docs/templates/templates.md embeds, so it
    # is already imported when pages/markdown.py parses the docs at startup.
    "/templates": {
        "module": "docs.templates.newsletter",
        "caption": "A complete newsletter, built from dash-email components",
        "max_height": 420,
    },
}
