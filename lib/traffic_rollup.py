...
Rollup v4 (sync item 12) adds the VENDOR dimension, additively, from the
ledger's second table — the ``reads`` rows dash-improve-my-llms 2.8.0 emits
for every corpus document it serves (``on_document_read``, kept by
...
def load_reads(path=None):
    ...
    A ledger written before sync item 12 has no ``reads`` key — absence is
    empty. No ``_SKIP`` filtering: the package only emits for documents it
    served, which are exactly the surfaces the visits reader skips.
    """
    import json
