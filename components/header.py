...
def create_link(icon, href, label, visible_from=None):
    ...
    does. The label lands on both the anchor and the button. ``visible_from``
    (a Mantine breakpoint) lets a link drop at phone widths where the header
    runs out of room.
    """
    return dmc.Anchor(
        dmc.ActionIcon(
...
def create_other_apps_menu():
    ...
    A hover menu in the top bar, populated from lib.network_directory:
    PEERS + AFFILIATED restricted to PRIMARY, this app omitted, labelled by
    domain. The sidebar carries no network section any more — this is the
    ...
    return dmc.Menu(
        [
            ...
            dmc.MenuDropdown(
                ...
                id="other-apps-menu",
                # Solid, themed panel: a transparent dropdown reads
    ...
    return dmc.AppShellHeader(
        dmc.Group(
            [
                ...
                # Right section: Email Builder CTA (this site's identity) +
                # backend badge + version badge + search + Other Apps +
