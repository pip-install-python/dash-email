...
NETWORK FILE: adapted from dash-documentation-boilerplate 1.2.4. It changes
only where this site legitimately differs — see the theme-colour test.
...
The two failures this repo actually shipped, both measured live on 2026-08-01:
...
1. **TWO empty og:image tags on every page.** No `register_page` call passed
   ...
   as the declared image and render a blank card. It was doubled because
   ...
   the whole template, comments included. The comment was invisible in a
   browser and perfectly visible to anything reading the raw HTML.

2. **A manifest naming another site.** `assets/favicon/site.webmanifest` was
   copied in from 2plot.dev and still read "2plot.dev — Dash Components
   Documentation" — the string an installed icon would carry on somebody's
   home screen forever.
...
def test_the_image_is_declared_exactly_once(client, page_paths):
    """The duplicate-tag regression, in both of the ways it happened here."""
    for path in page_paths[:8]:
...
def test_no_dash_placeholder_is_named_inside_a_comment():
    ...
    what put two empty og:image tags on every page of this site.
    """
...
def test_the_card_url_names_this_domain():
    ...
    assert OG_IMAGE_URL.endswith("/email.2plot.dev.png")
...
def test_the_rendered_card_on_disk_is_the_declared_shape():
    ...
    card = REPO_ROOT / "build" / "social-cards" / "email.2plot.dev.png"
    if not card.exists():
...
def test_no_meta_tag_dash_emits_is_also_declared_statically(client):
    ...
    `twitter:card` is the deliberate exception since the gate-wave pass:
...
def test_the_manifest_describes_THIS_site():
    """It shipped naming "2plot.dev", copied in from the hub.
...
def test_the_theme_colour_agrees_with_the_manifest(client):
    ...
    THIS SITE DIVERGES FROM THE TEMPLATE, deliberately: it declares two
    media-scoped theme-colours (the appshell follows `prefers-color-scheme`, so
    a single value paints the mobile chrome wrong for half of visitors) where
    the boilerplate declares one. The manifest can only carry one, and it is
    the dark surface — the same colour as `background_color`, which is what the
    install splash paints. So the assertion is membership rather than equality,
    and the light value having no manifest counterpart is correct.
    """
    manifest = json.loads(MANIFEST.read_text())
...
def test_every_asset_the_template_references_resolves(client):
    ...
    This repo is one `git add` away from the same thing: the favicon set landed
    in the same change as this test.
    """
    html = _visible(client.get("/").text)
