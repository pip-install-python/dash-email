...
Adopted from the template (sync item 18, 1.6.42); the positive-control
page is derived from the registry here rather than named, and the
source pin's composition sites are this fork's own: pages/markdown.py
concatenates `parse()`'s result (`section + layout`, already correct
here — this pin guards the pattern, not a known-broken line). This
fork's pages/home.py does not use markdown2dash at all (a hand-authored
DMC tree), so it carries no `parse`/`create_parser` call and is outside
this defect class entirely.
"""
...
def test_the_markdown_page_builder_concatenates_rather_than_nesting():
    ...
    fork's only markdown2dash composition site is pages/markdown.py — its
    `create_parser(directives)` result is bound to `parse`, and the pin
    checks the AST for any `[..., parse(...)]` list-literal nesting."""
