"""Rollup v4 — the vendor dimension, additive (sync item 12).
...
when reads are added.

The module under test is `lib.traffic_rollup`.
"""
...
from lib import traffic_rollup as R
...
def _read(path="/llms.txt", *, tier="index", vendor_key="gptbot",
    ...
    classify()-shaped event: the stored row carries exactly the package's
    EVENT_FIELDS, so a key the package does not emit — `vendor_class` on
    2.8.0 — is dropped here the way production drops it. A fixture written
    by hand would assert a shape production never produces, and the
    rollup would send `class: null` unnoticed."""
    import tempfile
...
def _rollup(tmp_path, monkeypatch, visits=(), reads=(), day=DAY):
    """`daily_rollup(app, day)` with the ledger in the env."""
...
def test_a_pre_item_12_ledger_has_no_reads_key_and_that_is_empty(tmp_path):
    p = tmp_path / "old.json"
...
def test_reads_are_joined_not_summed(tmp_path, monkeypatch):
    ...
    visits = [_visit("/", ip="2.2.2.2", ua="GPTBot/1.2", device_type="bot"),
              _visit("/getting-started", minute=1)]
...
def test_the_fixture_rows_are_what_record_read_stores():
    """The seam itself: a row is EVENT_FIELDS + kind, nothing else —
