...
def _declared_lastmods() -> set[str]:
    ...
    for md in Path("docs").glob("**/*.md"):
        ...
        if m:
            dates.add(m.group(1))
    # Generated pages declare theirs from their SOURCE (item 16, ported from
    # the template's 1.6.41): /changelog's is the newest dated release
    # heading in CHANGELOG.md, /api's the committed extract's `generated`
    # stamp — both move exactly when the content moves, which is what
    # "declared" means here.
    try:
