...
and does no I/O with it. Before sync item 12 nothing here listened, so
tier, verified, verdict, bytes and policy were discarded at the app
boundary exactly as the package used to discard them internally. These
pins drive the REAL app (conftest's client) and read the ledger file back.
"""
...
def test_a_browser_page_view_writes_no_read_row(app_module, client):
    """The package emits for the crawler document only; a Chrome GET / is
    the browser lane."""
    before = _reads(app_module)
...
def test_a_pre_item_12_ledger_gains_reads_without_losing_visits(tmp_path):
