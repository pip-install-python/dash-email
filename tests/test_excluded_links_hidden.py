...
def test_admin_paths_absent_from_sitemap_llms_and_sidebar(client, app):
    ...
    # The CORPUS, not only the index (sync item 18 consolidation 2 — the
    # gap this fork flagged as a spec/tree mismatch rather than inventing
    # the extension unilaterally; the template has since shipped it).
    # Prose leaks what structure hides: hyperlinking an admin path from a
    # docs page puts it into the corpus while every sitemap and sidebar
    # pin passes.
    #
    # LINKS only, deliberately: a changelog that cannot name the page it
    # added is not a changelog, and a mention in prose inside a code span
    # is not what makes a path REACHABLE — a link is. Both clauses are
    # link-shaped, including the machine path (`]({p}/llms.txt)`), so a
    # changelog entry naming `/admin/x/llms.txt` in prose does not trip
    # this pin for being honest about the fix.
