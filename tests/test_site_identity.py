...
What was live on email.2plot.dev until this pass: the /llms.txt H1 read a bare
**"# Dash Email"**, and every share card's headline read "Dash Email | <page>".
Neither is a framework default, and neither names anything a reader can act on
— there is no package called "Dash Email" on PyPI and no repository by that
...
# line, deliberately.
EXPECTED_BRAND = "dash-email — email components for Dash"
...
def test_home_prose_opens_with_the_brand():
    ...
    despite nothing in this repo importing it — which is exactly how its H1
    stayed at the old name after every other surface had been updated.
    """
    source = (REPO_ROOT / "pages" / "home.py").read_text()
    match = re.search(r'LLMS_DOC\s*=\s*f?"""(.*?)"""', source, re.S)
    assert match, "pages/home.py no longer defines LLMS_DOC"
    first = match.group(1).strip().splitlines()[0]
    # The literal is an f-string interpolating SITE_BRAND; either spelling of
    # the H1 is the same identity, and reading the source rather than importing
    # the module keeps this test free of Dash's page-registry side effects.
    assert first in (f"# {EXPECTED_BRAND}", "# {SITE_BRAND}")
...
def test_the_byline_is_in_the_description_not_the_brand():
    ...
    """
    assert "dash-email" in SITE_DESCRIPTION
...
def test_no_surface_still_carries_the_old_display_name():
    """A sweep, because the prefix was not the only place "Dash Email" hid.

    Comments and docstrings are stripped first: several of the files below
    ...
    thing from the site's name. `pages/home.py` is out for a more interesting
    reason: it carries the title of an embedded YouTube video, which is a real
    external title and must keep matching the video rather than this site. Its
    one identity-bearing string, `LLMS_DOC`'s H1, has a test of its own above.
    """
    offenders = []
