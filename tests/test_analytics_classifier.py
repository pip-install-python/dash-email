...
Until sync item 12 lib/analytics_tracker.py carried its own User-Agent lists:
it filed ClaudeBot (Anthropic's TRAINING crawler) under "search", still named
...
delegation in place — each UA string is one taken from the wire — and the
last test greps the module so a list cannot come back quietly.
"""
