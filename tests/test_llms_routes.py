...
def test_page_llms_txt_carries_the_page_prose(client):
    body = client.get("/getting-started/llms.txt").text
    assert "Getting Started" in body
    assert "pip install dash-email" in body, "page prose did not reach /<page>/llms.txt"
...
def test_source_directives_are_expanded_in_llms_txt(client):
    ...
    assert "import dash_email as de" in body, "the referenced source file was not inlined"
...
def test_robots_artifact_fingerprint(client):
    ...
    # POSTURE, not artifact, since sync item 15 (the wall retired; training
    # crawlers allowed by default): with block_ai_training=False the
    ...
    # instead and records the divergence.
...
def test_fastapi_healthz_renders_from_the_shared_payload(monkeypatch):
    ...
    This fork is Flask-only and carries no lib/asgi_routes.py, so BOTH
    importorskips fire today — the test is here so a future backend port
    inherits the pin instead of re-deriving it (the same reason
    requirements.txt keeps the commented [fastapi]/[quart] floor lines).
    """
    fastapi = pytest.importorskip("fastapi")
    pytest.importorskip("lib.asgi_routes")
...
# Any docs page works here; none of this site's pages documents the viewer,
# so there is no page whose prose legitimately names "dv-banner".
PAGE_DOC = "/components/email-structure/llms.txt"
...
def test_a_page_may_document_the_viewer_without_tripping_the_check():
    """Prose that *names* `dv-banner` is content, not chrome.

    On the template the /networks page documents the viewer and pins this
    live; this fork has no such page, so the distinction is pinned against
    the pattern directly — the check can't be "fixed" later by making it
    substring-based again without failing here.
    """
    prose = 'The viewer banner is a <div> carrying the class name "dv-banner".'
    assert "dv-banner" in prose
    assert not CHROME.search(prose), "a plain-text mention must not read as chrome"
    assert CHROME.search('<header class="dv-banner">'), "real chrome must still match"
